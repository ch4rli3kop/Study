...
/*
   ...
   NONCONTIGOUS_BIT는 MORECORE(brk)의 동작 과정에서 이전 메모리 영역과 연속적인 메모리 영역이 리턴되지 않았을 경우를 나타낸다.
 */
...
__libc_malloc (size_t bytes)
{
  ...
  if (__builtin_expect (hook != NULL, 0)) // hook이 등록되어 있으면 call
  ...
  arena_get (ar_ptr, bytes);  //  main_arena mutex도 설정함.
  victim = _int_malloc (ar_ptr, bytes);  //  malloc 동작의 핵심임.
  /* Retry with another arena only if we were able to find a usable arena
     before.  사용가능한 다른 arena를 찾음*/
  if (!victim && ar_ptr != NULL)
    {
      LIBC_PROBE (memory_malloc_retry, 1, bytes);
      ar_ptr = arena_get_retry (ar_ptr, bytes);  // 다른 arena 찾기
      victim = _int_malloc (ar_ptr, bytes);  //  malloc 수행
    }
  if (ar_ptr != NULL)
    __libc_lock_unlock (ar_ptr->mutex);  //  mutex 해제
  assert (!victim || chunk_is_mmapped (mem2chunk (victim)) ||
          ar_ptr == arena_for_chunk (mem2chunk (victim)));  //  mmap으로 할당한 chunk는 munmap으로 chunk 해제하기 때문에, list에 안들어감.
  return victim;  //  해당 chunk를 사용자에게 리턴함.
}
...
_int_malloc (mstate av, size_t bytes)
{
  ...
  /*
     ...
     aligned.
     SIZE_SZ bytes의 추가적인 overhead를 얻고, 사용가능한 alignment 단위로 나타내기 위해서, 사용자가 요청한 bytes를 nb로 바꿔준다. 최소 MINSIZE보다 작은 요청의 경우, MINSIZE를 리턴해준다.
   */
  checked_request2size (bytes, nb);


  /* There are no usable arenas.  Fall back to sysmalloc to get a chunk from mmap.
     top chunk에 사용가능한 공간이 없을 경우, sysmalloc을 호출하여 시스템에 메모리를 요구한다. sysmalloc에서는 mmap을 호출하는데, 이를 통해서 top chunk의 크기를 확장하거나 대체하여 동적할당 영여을 넓힐 수 있다. 기존 old top chunk 영역을 free 시키고 새로 mmap함. 정확히 이 동작은 여기서 하는게 아님.
  */
  ...
  /*
   ...
   This code is safe to execute even if av is not yet initialized, so we can try it without checking, which saves some time on this fast path.
   다음은 size가 fastbin 범위에 들어갈 경우에 동작한다.
  ...
  if ((unsigned long) (nb) <= (unsigned long) (get_max_fast ()))
    {
      ...
      if (victim != NULL) // NULL인 경우 small bin으로 넘어감
        {
          size_t victim_idx = fastbin_index (chunksize (victim)); // 꺼내욘 victim의 chunk size가 실제 그 bin에 맞는 크기인지 검사한다. *exploit 시 fastbin의 chunk size를 맞춰줘야하는 이유*
          if (victim_idx != idx)
